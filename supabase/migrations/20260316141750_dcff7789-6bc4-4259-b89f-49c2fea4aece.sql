...
BEGIN
  ...
  PERFORM public.enqueue_email('transactional_emails', jsonb_build_object(
    ...
    'text','Hi ' || COALESCE(_customer_name,'there') || ', ' || COALESCE(_artisan_name,'the artisan') || ' completed "' || LEFT(NEW.title,60) || '". Review within 48 hours: ' || _app_url || '/jobs/' || NEW.id,
...
BEGIN
  ...
  PERFORM public.enqueue_email('transactional_emails', jsonb_build_object(
    'run_id','464c5308-da8a-4d9a-938f-7a0ea86aec4b','to',_artisan_email,'from','Jobman <support@jobman.ng>','sender_domain','notify.www.jobman.ng',
    'subject','Payment Received — NGN ' || NEW.amount::text || ' for "' || COALESCE(LEFT(_job_title,40),'a job') || '"','html',_html,
    'text','Hi ' || COALESCE(_artisan_name,'there') || ', NGN ' || NEW.amount::text || ' released to your wallet for "' || COALESCE(LEFT(_job_title,60),'a job') || '". View: ' || _app_url || '/wallet',
