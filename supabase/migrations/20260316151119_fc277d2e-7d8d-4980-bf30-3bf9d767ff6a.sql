-- Update all email trigger functions to use www.jobman.ng instead of jobmanmarketplace.lovable.app
...
BEGIN
  ...
  PERFORM net.http_post(
    ...
    body := jsonb_build_object('to',_email,'subject','Welcome to Jobman! 🎉','eventType','welcome','details',jsonb_build_object('name',_name,'link','https://www.jobman.ng/dashboard','message','Welcome to Jobman — Nigeria''s trusted marketplace connecting skilled artisans with customers who need quality work done.'))
  );
  RETURN NEW;
END;
$function$;
...
BEGIN
  ...
  PERFORM net.http_post(
    url := 'https://bpuvhiifnbuytzzbgfln.supabase.co/functions/v1/send-email-notification',
    headers := jsonb_build_object('Content-Type','application/json','Authorization','Bearer ' || current_setting('supabase.service_role_key', true)),
    body := jsonb_build_object('to',_email,'subject','🔔 Complete Your Verification to Start Earning','eventType','verification_reminder','details',jsonb_build_object('name',COALESCE(_name,'there'),'link','https://www.jobman.ng/artisan-verification','message','You are one step away from receiving jobs and earning on Jobman. Complete your ID verification to unlock all features.'))
  );
  RETURN NEW;
END;
$function$;
